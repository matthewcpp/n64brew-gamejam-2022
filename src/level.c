...
int fw64_level_raycast(fw64Level* level, Vec3* origin, Vec3* direction, uint32_t mask, fw64RaycastHit* hit) {
    ...
    for (uint32_t i = 0; i < level->chunk_ref_count; i++) {
        ...
        if (!fw64_collision_test_ray_box(origin, direction, &ref->scene->bounding_box, &scene_hit.point, &scene_hit.distance))
            continue;
...
int fw64_level_moving_spheres_dynamic_intersection(fw64Level* level, Vec3* center, float radius, Vec3* velocity, uint32_t mask, fw64IntersectMovingSphereQuery* result) {
    ...
    for (uint32_t i = 0; i < node_count; i++) {
        ...
        if(fw64_collision_test_moving_spheres(center, radius, velocity, &node_center, node_radius, &node_vel, &out_t)) {
            ...
            Vec3 vel_norm;
            vec3_copy(velocity, &vel_norm);
            vec3_normalize(&vel_norm);
            vec3_add_and_scale(center, &vel_norm, out_t, &sphere_intersect->point);
            ++result->count;
        }
...
void fw64_level_draw_camera_all(fw64Level* level, fw64RenderPass* renderpass, fw64Camera* camera) {
    fw64_level_draw_camera_layer_mask(level, renderpass, camera, FW64_LAYER_MASK_ALL_LAYERS);
}

void fw64_level_draw_camera_layer_mask(fw64Level* level, fw64RenderPass* renderpass, fw64Camera* camera, fw64LayerMask layer_mask) {
    fw64Frustum frustum;
    ...
    for (uint32_t i = 0; i < level->chunk_ref_count; i++) {
        ...
        fw64_scene_draw_all(ref->scene, renderpass, layer_mask);
    }

    for (uint32_t i = 0; i < level->dynamic_node_count; i++) {
        fw64Node* node = level->dynamic_nodes[i];
        if (!node->mesh_instance || !(node->layer_mask & layer_mask)) {
            continue;
        }
